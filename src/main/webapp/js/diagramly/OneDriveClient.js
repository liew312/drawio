...
(function () {
  ...
  window.OneDriveClient = function (
    editorUi,
    isExtAuth,
    inlinePicker,
    noLogout
    ...
    if (
      ...
      ((window.urlParams != null && window.urlParams["inlinePicker"] == "1") ||
        mxClient.IS_ANDROID ||
        mxClient.IS_IOS) //Mobile devices doesn't work with OneDrive picker, so, use the inline picker
    ) {
      inlinePicker = true;
    }
    ...
    DrawioClient.call(
      this,
      editorUi,
      isExtAuth ? "oneDriveExtAuthInfo" : "oneDriveAuthInfo"
    );
    ...
    if (authInfo != null) {
      this.endpointHint =
        authInfo.endpointHint != null
          ? authInfo.endpointHint.replace(
              "/Documents",
              "/_layouts/15/onedrive.aspx"
            )
          : authInfo.endpointHint;
    }
  };
  ...
  OneDriveClient.prototype.clientId =
    ...
    (window.location.hostname == "test.draw.io"
      ? "2e598409-107f-4b59-89ca-d7723c8e00a4"
      : "45c10911-200f-4e27-a666-9e9fca147395");

  OneDriveClient.prototype.clientId =
    window.location.hostname == "app.diagrams.net"
      ? "b5ff67d6-3155-4fca-965a-59a3655c4476"
      : OneDriveClient.prototype.clientId;

  OneDriveClient.prototype.clientId =
    window.location.hostname == "viewer.diagrams.net"
      ? "417a451a-a343-4788-b6c1-901e63182565"
      : OneDriveClient.prototype.clientId;
  /**
   * OAuth 2.0 scopes for installing Drive Apps.
   */
  OneDriveClient.prototype.scopes =
    "user.read files.readwrite.all sites.read.all";

  /**
   * OAuth 2.0 scopes for installing Drive Apps.
   */
  OneDriveClient.prototype.redirectUri =
    window.location.protocol + "//" + window.location.host + "/microsoft";
  OneDriveClient.prototype.pickerRedirectUri =
    window.location.protocol + "//" + window.location.host + "/onedrive3.html";

  /**
   * This is the default endpoint for personal accounts
   */
  OneDriveClient.prototype.defEndpointHint = "api.onedrive.com";
  OneDriveClient.prototype.endpointHint =
  ...
  OneDriveClient.prototype.resetTokenRefresh = function (expires_in) {
    ...
    if (expires_in > 0) {
      ...
      this.tokenRefreshThread = window.setTimeout(
        mxUtils.bind(this, function () {
   ...
   */
  OneDriveClient.prototype.authenticate = function (
    success,
    error,
    failOnAuth
  ) {
    if (this.isExtAuth) {
      window.parent.oneDriveAuth(
        ...
        error,
        window.urlParams != null && urlParams["odAuthCancellable"] == "1"
      );
      return;
    }
  ...
  OneDriveClient.prototype.updateAuthInfo = function (
    newAuthInfo,
    remember,
    forceUserUpdate,
    success,
  ...
  OneDriveClient.prototype.authenticateStep2 = function (
    state,
    success,
    error,
    failOnAuth
      ...
      var auth = mxUtils.bind(this, function () {
        ...
        if (authInfo != null) {
          var req = new mxXmlRequest(
            ...
            null,
            "GET"
          ); //To identify which app/domain is used

          req.send(
            mxUtils.bind(this, function (req) {
              if (req.getStatus() >= 200 && req.getStatus() <= 299) {
                this.updateAuthInfo(
                  JSON.parse(req.getText()),
                  authInfo.remember,
                  false,
                  success,
                  error
                ...
                if (req.getStatus() == 401 && !failOnAuth) {
                  // (Unauthorized) [e.g, invalid refresh token]
                  auth();
                } else {
                  error({
          ...
          this.ui.showAuthDialog(
            this,
            true,
            mxUtils.bind(this, function (remember, authSuccess) {
              var url =
                ...
                "&scope=" +
                encodeURIComponent(
                  this.scopes + (remember ? " offline_access" : "")
                ) +
                "&state=" +
                encodeURIComponent(
                  "cId=" +
              ...
              if (popup != null) {
                window.onOneDriveCallback = mxUtils.bind(
                  this,
                  function (authInfo, authWindow) {
  ...
  OneDriveClient.prototype.getAccountTypeAndEndpoint = function (
    success,
    error
  ) {
    this.get(
      this.baseUrl + "/me/drive/root",
      mxUtils.bind(this, function (req) {
        try {
          if (req.getStatus() >= 200 && req.getStatus() <= 299) {
            ...
            if (resp.webUrl.indexOf(".sharepoint.com") > 0) {
              ...
              this.endpointHint = resp.webUrl.replace(
                "/Documents",
                "/_layouts/15/onedrive.aspx"
              );
            } else {
  ...
  OneDriveClient.prototype.executeRequest = function (url, success, error) {
        ...
        mxUtils.bind(this, function (req) {
          ...
          if (acceptResponse) {
            ...
            // 400 is returns if wrong user for this file
              ...
              !failOnAuth &&
              (req.getStatus() === 401 || req.getStatus() === 400)
            ) {
    ...
    if (_token == null || this.tokenExpiresOn - Date.now() < 60000) {
      //60 sec tolerance window
  ...
  OneDriveClient.prototype.checkToken = function (fn) {
    if (
      _token == null ||
      this.tokenRefreshThread == null ||
      this.tokenExpiresOn - Date.now() < 60000
    ) {
  ...
  OneDriveClient.prototype.removeExtraHtmlContent = function (data) {
    var idx = data.lastIndexOf(
      '<html><head><META HTTP-EQUIV="Content-Type" CONTENT="text/html; charset=utf-8"><meta name="Robots" '
    );
   ...
   * Checks if the client is authorized and calls the next step.
   */
  OneDriveClient.prototype.getFile = function (
    id,
    success,
    error,
    denyConvert,
                ...
                try {
                  ...
                  if (acceptResponse) {
                    ...
                    // Checks for base64 encoded mxfile
                      ...
                      data.substring(0, 32) ==
                      "data:image/png;base64,PG14ZmlsZS"
                    ) {
   ...
   */
  OneDriveClient.prototype.renameFile = function (
    file,
    filename,
    success,
    error
      ...
      if (!this.isValidFilename(filename)) {
        error({
          message: this.invalidFilenameRegExs[0].test(filename)
            ? mxResources.get("oneDriveCharsNotAllowed")
    ...
    if (folderInfo.driveId != fileInfo.driveId) {
      error({
        message: mxResources.get(
          "cannotMoveOneDrive",
          null,
          "Moving a file between accounts is not supported yet."
        ),
      });
    } else {
      this.writeFile(
        this.getItemURL(id),
        JSON.stringify({ parentReference: folderInfo }),
        "PATCH",
        "application/json",
        success,
        error
   ...
   * @param {number} dy Y-coordinate of the translation.
   */
  OneDriveClient.prototype.insertLibrary = function (
    filename,
    data,
    success,
    error,
   ...
   * @param {number} dy Y-coordinate of the translation.
   */
  OneDriveClient.prototype.insertFile = function (
    filename,
    data,
    success,
    error,
        ...
        if (checked) {
          ...
          if (data.length >= 4000000 /*4MB*/) {
            ...
            this.writeFile(
              ...
              "PUT",
              null,
              mxUtils.bind(this, function (meta) {
                this.writeLargeFile(
                  this.getItemURL(meta.id),
                  data,
                  insertSuccess,
                  error
                );
   ...
   * @param {number} dy Y-coordinate of the translation.
   */
  OneDriveClient.prototype.checkExists = function (
    parentId,
    filename,
    askReplace,
    fn
    ...
    this.executeRequest(
      this.baseUrl + folder + "/children/" + encodeURIComponent(filename),
      mxUtils.bind(this, function (req) {
        if (req.getStatus() == 404) {
          ...
          if (askReplace) {
            ...
            this.ui.confirm(
              mxResources.get("replaceIt", [filename]),
              function () {
                fn(true);
              },
              function () {
    ...
    try {
      ...
      if (this.ui.useCanvasForExport && /(\.png)$/i.test(file.meta.name)) {
        ...
        this.ui.getEmbeddedPng(
          ...
          error,
          this.ui.getCurrentFile() != file ? savedData : null,
          p.scale,
          p.border
        );
      } else {
  ...
  OneDriveClient.prototype.writeLargeFile = function (
    url,
    data,
    success,
    error,
    ...
    try {
      ...
      if (data != null) {
        var uploadPart = mxUtils.bind(
          this,
          function (uploadUrl, index, retryCount) {
            try {
   ...
   * @param {number} dy Y-coordinate of the translation.
   */
  OneDriveClient.prototype.writeFile = function (
    url,
    data,
    method,
    contentType,
  ...
  OneDriveClient.prototype.createInlinePicker = function (fn, foldersOnly) {
    return mxUtils.bind(this, function () {
      ...
      var dlg = new CustomDialog(
        ...
        null,
        mxResources.get(foldersOnly ? "save" : "open"),
        null,
        null,
        null,
        null,
      ...
      odPicker = new mxODPicker(
        div,
        null,
        mxUtils.bind(this, function (url, success, error) {
          this.executeRequest(
            this.baseUrl + url,
            function (req) {
              success(JSON.parse(req.getText()));
        ...
        mxUtils.bind(this, function (id, driveId, success, error) {
          this.executeRequest(
            this.baseUrl + "/drives/" + driveId + "/items/" + id,
            function (req) {
              success(JSON.parse(req.getText()));
            },
            error
        ...
        function (item) {
          if (foldersOnly) {
            //Currently this is not called when in foldersOnly mode
            fn({
              value: [item],
            });
          } else {
  ...
  OneDriveClient.prototype.pickFile = function (fn) {
    ...
    var odOpenDlg = this.inlinePicker
      ? this.createInlinePicker(fn)
      : mxUtils.bind(this, function () {
          OneDrive.open({
            clientId: this.clientId,
            action: "query",
  ...
  OneDriveClient.prototype.logout = function () {
    ...
    this.ui.editor.loadUrl(
      this.redirectUri +
        "?doLogout=1&state=" +
        encodeURIComponent(
          "cId=" + this.clientId + "&domain=" + window.location.hostname
        )
    );
    this.clearPersistentToken();
    this.setUser(null);
